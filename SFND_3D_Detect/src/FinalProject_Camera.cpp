       ...
       // continue; // skips directly to the next image without processing what comes beneath
        ...
        if (bVis)
        {
            ...
            for (auto it2 = (dataBuffer.end() - 1)->boundingBoxes.begin(); it2 != (dataBuffer.end() - 1)->boundingBoxes.end(); ++it2)
            {
                ...
                left = (*it2).roi.x;
                width = (*it2).roi.width;
                height = (*it2).roi.height;
                cv::rectangle(visImg, cv::Point(it2->roi.x, it2->roi.y), cv::Point(it2->roi.x + it2->roi.width, it2->roi.y + it2->roi.height), cv::Scalar(0, 255, 0), 2);
                string label = "car:" + to_string(it2->boxID);
                int baseLine;
